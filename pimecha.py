...
'''
This file contains PiMecha servo serial codes
...
class PiMecha(SerialComm):
    ...
    def connect(self, port, baudrate=115200):
        ...
        self.log.info('Connecting to PiMecha on Port %s & baudrate %d..', port, baudrate)
        status = super(PiMecha, self).connect(port, baudrate)
        if status == True:
            self.log.info('PiMecha is Initialized')
        else:
            self.log.info('PiMecha Initialization Failed..!!')
